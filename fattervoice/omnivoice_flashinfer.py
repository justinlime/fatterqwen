"""Vendored FlashInfer acceleration patch for OmniVoice."""

# NOTE: This file is vendored verbatim from k2-fsa/OmniVoice (master branch,
# `omnivoice/models/omnivoice_flashinfer.py`), which is NOT included in any
# released omnivoice PyPI package (checked 0.1.5, 0.2.0, 0.2.1). It is Apache-2.0
# licensed and the original copyright header is preserved below. The code imports
# only helpers that exist in the pinned `omnivoice==0.1.5`
# (GenerationTask, OmniVoiceGenerationConfig, _get_time_steps, _gumbel_sample) and
# monkeypatches OmniVoice._generate_iterative at runtime via `apply_flashinfer(...)`.
# Keep this file in sync with upstream when upgrading omnivoice.
